...
import { IPrompts, Prompts } from '../Models/prompts_model';
...
export class PromptsService {
    ...
    async getPrompts(): Promise<IPrompts[]> {
        const populated = await Prompts.find().populate('category_id').populate('sub_category_id').exec();
        if (!populated) throw new Error("Could not get prompts!");
        return populated;
    }
